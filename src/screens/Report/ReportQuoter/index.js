...
import {Stack, Image, HStack, Text, Divider, VStack} from 'native-base';
import {useDispatch, useSelector} from 'react-redux';
import {Text as TextReact} from 'react-native';
import {styles} from '../../../components/styles';
import {useFocus} from 'native-base/src/components/primitives';
				...
				<HStack space={2}>
					...
					<HStack>
						...
						<VStack space={1}>
							...
							<Stack>
								<TextReact style={{fontSize: 17, fontWeight: 'bold', color: colors.dark}}>$ {total === 0 ? 0 : total.toFixed(0).toString().replace(/\B(?=(\d{3})+(?!\d))/g, '.')}
								</TextReact>
							</Stack>
						</VStack>
					</HStack>
					...
					<HStack space={2}>
						...
						<HStack space={5}>
							...
							<Stack justifyContent={'center'} w={180} alignItems={'flex-end'}>
								<TextReact style={styles.textMedium}>{preferences === 0 ? 'No hay datos' : preferences.storeName}</TextReact>
							</Stack>
						</HStack>
					</HStack>
					<HStack space={2}>
						...
						<HStack space={5}>
							...
							<Stack justifyContent={'center'} w={125} alignItems={'flex-end'}>
								<TextReact style={styles.textMedium}>{preferences === 0 ? 'No hay datos' : preferences.constructionName}</TextReact>
							</Stack>
						</HStack>
					</HStack>
					<HStack space={2}>
						...
						<HStack space={5}>
							...
							<Stack justifyContent={'center'} w={180} alignItems={'flex-end'}>
								<TextReact style={styles.textMedium}>{preferences === 0 ? 'No hay datos' : preferences.materialName}</TextReact>
							</Stack>
						</HStack>
					</HStack>
				</Stack>
