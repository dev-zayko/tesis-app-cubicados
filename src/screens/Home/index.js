...
} from '../../redux/features/Memberships/membershipsSlice';
...
const Home = ({navigation}) => {
  ...
  const {memberships, loading: loadingMembership} = useSelector(state => ({
    ...state.memberships,
  }));
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      dispatch(getPopularStores({})).then(response => {
        MostPopularStore(response.payload.data);
      });
      dispatch(getPopularMemberships({})).then(response => {
  ...
  const MostPopularPlans = popularMemberships => {
    const {membresiasMensuales, membresiasTrimestrales, membresiasAnuales} =
      popularMemberships[0];
    const maxVal = Object.entries({
      membresiasMensuales,
      membresiasTrimestrales,
      membresiasAnuales,
    }).sort(([, a], [, b]) => b - a)[0];
  ...
  const MostPopularStore = popularStore => {
    ...
    const data = myArray.sort(function (a, b) {
  ...
  const MostPopularTrademark = (popularTrademark, idConstructionType) => {
    const data = popularTrademark.sort(function (a, b) {
      return a.count - b.count;
    });
  ...
  return (
    <Background>
      <Container>
        <Image
          source={require('../../assets/banner-projects.jpg')}
          borderRadius={10}
          w={350}
          h={'100%'}
          alt={'banners'}
          ...
          <ScrollView>
            ...
            <Stack w={'100%'} alignItems={'center'} h={70}>
              <Text color={'white'} fontSize={18}>
                Bienvenido a cubicados, una app que se creo para ayudarte a ti y
                a tu negocio
              </Text>
            </Stack>
            <Stack backgroundColor={'yellow.500'} h={320}>
              ...
              <Stack alignItems={'center'}>
                <HStack h={'100%'} w={'90%'} space={5}>
                  <VStack space={2}>
                    <Image
                      source={require('../../assets/plano.png')}
                      w={58}
                      h={58}
                      alt={'plano'}
                    />
                  ...
                  <VStack w={200} space={5}>
                    <Text fontSize={18} color={'white'}>
                      Cubica tus proyectos
                    </Text>
                    <Text fontSize={18} color={'white'}>
                      Exporta tus cubicaciones
                    </Text>
                    <Text fontSize={18} color={'white'}>
                      Membresias al mejor precio
                    </Text>
                    <Text fontSize={18} color={'white'}>
                      Algoritmo que busca productos en tiempo real
                    </Text>
                  </VStack>
                </HStack>
              </Stack>
                ...
                <Stack alignItems={'center'} w={'100%'}>
                  <Stack w={'100%'}>
                    ...
                    <VStack alignItems={'center'} w={'100%'}>
                      <Text fontSize={20} color={'white'}>
                        {memberships[memberPopular]?.name}
                      </Text>
                    </VStack>
                  </Stack>
              ...
              <Stack w={'100%'} h={300}>
                ...
                <Stack w={'100%'} left={5}>
                  <Text color={'white'} fontSize={15}>
                    Los Usuarios estan comprando en
                  </Text>
                </Stack>
                <Stack left={5}>
                  <Text fontSize={18} color={'white'}>
                    {storePopular[2]?.name}: {storePopular[2]?.count}{' '}
                    cotizaciones
                  </Text>
                  <Text fontSize={18} color={'white'}>
                ...
                <Stack
                  ...
                  {storePopular[2]?.name === 'Construmart' ? (
                    <Image
                      source={require('../../assets/store-construmart.jpg')}
                      w={100}
                      borderRadius={100}
                      h={100}
                      alt={'images'}
                    ...
                    <Image
                      source={require('../../assets/store-sodimac.png')}
                      w={100}
                      borderRadius={100}
                      h={100}
                      alt={'imagev'}
            ...
            <Stack w={'100%'} h={250}>
              ...
              <Stack space={2}>
                <Stack left={5}>
                  <Text color={'white'} fontSize={15}>
                    En superficies lidera:
                  </Text>
                  <Text fontSize={18} color={'white'}>
                    {surfacePopular[2]?.trademark}: {surfacePopular[2]?.count}{' '}
                    cotizaciones
                  </Text>
                  <Text fontSize={18} color={'white'}>
                    {surfacePopular[1]?.trademark}: {surfacePopular[1]?.count}{' '}
                    cotizaciones
                  </Text>
                  <Text fontSize={18} color={'white'}>
                    {surfacePopular[0]?.trademark}: {surfacePopular[0]?.count}{' '}
                    cotizaciones
                  </Text>
                </Stack>
                <Stack left={5}>
                  <Text color={'white'} fontSize={15}>
                    En Revestimiento lidera:
                  </Text>
                  {coatingPopular[2]?.trademark !== undefined && (
                    <Text fontSize={18} color={'white'}>
                      {coatingPopular[2]?.trademark}: {coatingPopular[2]?.count}{' '}
                      cotizaciones
                    </Text>
                  )}
                  <Text fontSize={18} color={'white'}>
                    {coatingPopular[1]?.trademark}: {coatingPopular[1]?.count}{' '}
                    cotizaciones
                  </Text>
                  <Text fontSize={18} color={'white'}>
                    {coatingPopular[0]?.trademark}: {coatingPopular[0]?.count}{' '}
                    cotizaciones
                  </Text>
                </Stack>
              </Stack>
            ...
            <Stack w={'100%'} h={250}>
              ...
              <Stack space={2} alignItems={'center'}>
                <TouchableOpacity
                  ...
                  onPress={() =>
                    Linking.openURL(
                      'http://www.polpaico.cl/wp-content/uploads/NUEVO_VOL_DOSIFICACION-LEY.pdf',
                    )
                  }>
                ...
                <TouchableOpacity
                  ...
                  onPress={() =>
                    Linking.openURL(
                      'https://cbb.cl/wp-content/uploads/2022/04/Tabla-de-dosificacion.png',
                    )
                  }>
            ...
            <Stack w={'100%'} h={250}>
              ...
              <Stack
                ...
                h={100}>
                <TouchableOpacity
                  onPress={() => Linking.openURL('https://www.cubicados.cl/')}>
                  <Text underline color={colors.brand} fontSize={18}>
                    https://www.cubicados.cl/
                  </Text>
                </TouchableOpacity>
              </Stack>
            </Stack>
