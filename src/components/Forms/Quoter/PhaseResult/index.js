...
const PhaseQuoterResult = props => {
  ...
  const {productSelect, typeProduct} = useSelector(state => ({
    ...state.products,
  }));
  ...
  const castPrice = price => {
    ...
    if (gallonNeeded > 2) {
      ...
      setTotalCost(
        (gallonNeeded * price).toString().replace(/\B(?=(\d{3})+(?!\d))/g, '.'),
      );
    } else {
      if (gallonNeeded === 0) {
        setCountGallon(1);
  ...
  const matchingCommune = () => {
    ...
    dispatch(matchCommune({commune: city})).then(response => {
      addMaterial(response.payload.data[0].id);
    });
  };
  const confirmCubage = () => {
    Alert.alert('Aviso', '¿Estas seguro de agregar esta cubicación ?', [
      {
        text: 'Cancelar',
      ...
      <Stack space={3} w={'90%'}>
        ...
        <HStack space={2} alignItems={'center'}>
          ...
          <TextReact style={styles.textLarge}>
            {constructionTypeSelect.id === 1
              ? `${m3} m3`
              : constructionTypeSelect.id === 2 && `${area} m2`}
          </TextReact>
        </HStack>
        <VStack>
          <HStack
        ...
        {constructionTypeSelect.id === 1 ? (
          <>
            ...
            <VStack>
              <HStack space={'20%'}>
                <StyleText>Grava</StyleText>
                <HStack space={2}>
                  <StyleText>Arena</StyleText>
                  <Text fontSize={10} top={2}>
                    (5mm)
                  </Text>
                </HStack>
                <StyleText>Agua</StyleText>
              </HStack>
              <HStack space={20}>
                <Image
