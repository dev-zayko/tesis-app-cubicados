...
import {getProducts, resetListProducts, setProductSelect} from "../../../redux/features/Quoter/quoterSlice";
...
const ListProducts = () => {
  ...
  const {products, loading, typeProduct, status} = useSelector(state => ({...state.products}));
  const {user} = useSelector(state => ({...state.auth}));
  const {define, mineRegion, mineCommune} = useSelector(state => ({...state.utility}));
  ...
  const onSearchProducts = () => {
    ...
    dispatch(getProducts({token: user, storeName: name, typeProduct: typeProduct, region: define === 'other' ? regionSelect : mineRegion, commune: define === 'other' ? communeSelect : mineCommune.toUpperCase()})).then(() => setDisabledSelect(false));
  }
  useEffect(() => {
    console.log(define)
    if (define === 'mine') {
        ...
        <Stack>
          ...
          <VStack w={'100%'} h={100} alignItems={'center'} space={5}>
            <>
              ...
              <ListCommune setCommune={(commune) => {
          ...
          <Stack alignItems={'center'}>
            {loading === false ?
              <TouchableOpacity style={styles.buttonLogin} onPress={() => onSearchProducts()}>
